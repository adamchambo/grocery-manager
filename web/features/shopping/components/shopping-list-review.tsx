...
export function ShoppingListReview() {
  ...
  async function saveSuggestion(item: ShoppingListItemResponse, raw: string) {
    ...
    try { await putApiShoppingListsListIdItemsItemId(listId, item.id, { suggestedPurchaseQuantity: quantity, actualPurchaseQuantity: item.actualPurchaseQuantity, outcome: item.outcome, destinationLocationId: item.destinationLocationId, version: item.version }); await load() } catch (value) { setError(getApiErrorMessage(value, "Unable to update quantity.")) }
  }
...
function ManualItemForm({ listId, onAdded, onError }: Readonly<{ listId: string; onAdded: () => Promise<void>; onError: (value: string) => void }>) {
  ...
  async function add() { const numeric = Number(quantity); if (!name.trim() || !Number.isFinite(numeric) || numeric <= 0) { onError("Enter an item name and quantity greater than zero."); return } setSaving(true); try { await postApiShoppingListsListIdItems(listId, { name: name.trim(), suggestedPurchaseQuantity: numeric, createPantryItemOnPurchase: false, pantryCategoryId: null, pantryTrackingUnit: null, destinationLocationId: null }); setName(""); setQuantity("1"); showSuccessToast("One-off item added"); await onAdded() } catch (value) { onError(getApiErrorMessage(value, "Unable to add item.")) } finally { setSaving(false) } }
