...
export function StocktakeChecklist() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><header className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold">Stocktake</h1><p className="mt-1 text-sm text-muted-foreground">Count what you have in each area.</p></div><Button variant="outline" onClick={() => router.replace("/app/stocktakes")}>Exit</Button></header><div className="space-y-2"><div className="flex items-center justify-between text-sm"><span className="font-medium">{area?.name}</span><span className="text-muted-foreground">Area {areaIndex + 1} of {areas.length}</span></div><Progress value={areas.length ? ((areaIndex + 1) / areas.length) * 100 : 0} /></div><Card><CardHeader><CardTitle>{area?.name}</CardTitle><CardDescription>Enter the quantity you can see. Items appear here only because they are stored in this area.</CardDescription></CardHeader><CardContent className="space-y-3">{area?.entries.map((entry) => <div key={entry.id} className="rounded-xl bg-surface-muted/55 px-4 py-3 sm:flex sm:items-center sm:justify-between sm:gap-6"><div><FieldLabel htmlFor={`quantity-${entry.id}`} className="font-medium text-foreground">{entry.itemName}</FieldLabel><p className="mt-0.5 text-xs text-muted-foreground">Count in {entry.trackingUnit.toLowerCase()}</p></div><div className="mt-2 flex items-center gap-2 sm:mt-0"><Input id={`quantity-${entry.id}`} aria-label={`${entry.itemName} quantity in ${entry.trackingUnit}`} type="number" min="0" step="0.001" value={drafts[entry.id] ?? ""} disabled={saving} onChange={(event) => setDrafts((value) => ({ ...value, [entry.id]: event.target.value }))} className="w-28" /><span className="text-sm text-muted-foreground">{entry.trackingUnit.toLowerCase()}</span></div></div>)}</CardContent></Card><div className="flex justify-between gap-3"><Button variant="outline" disabled={areaIndex === 0 || saving} onClick={() => setAreaIndex((value) => value - 1)}>Back</Button><Button disabled={!area || saving} onClick={() => void saveArea()}>{saving && <Spinner />}{areaIndex < areas.length - 1 ? "Next area" : "Generate shopping list"}</Button></div></div>
}
